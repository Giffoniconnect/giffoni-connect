...
>(
  (
    ...
    const [_open, _setOpen] = React.useState(() => {
      if (isClient) {
        return document.cookie.includes(`${SIDEBAR_COOKIE_NAME}=true`) || defaultOpen;
      }
      return defaultOpen;
    })
    ...
    const collapsible = isClient && document.cookie.includes(`${SIDEBAR_COOKIE_NAME}=true`) ? 'none' : 'icon';

    // We add a state so that we can do data-state="expanded" or "collapsed".
    // This makes it easier to style the sidebar with Tailwind classes.
    const state = open || (isHovering && collapsible === 'icon') ? "expanded" : "collapsed";
  ...
  (
    ...
    return (
      ...
      >
        <div
          className={cn(
            "fixed inset-y-0 z-10 hidden h-svh md:flex flex-col transition-[width] duration-300 ease-in-out bg-sidebar",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-hover/sidebar-wrapper:group-data-[state=collapsed]:mt-0 group-hover/sidebar-wrapper:group-data-[state=collapsed]:opacity-100",
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-hover/sidebar-wrapper:group-data-[state=collapsed]:flex",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
    "peer/menu-button flex w-full items-center justify-start gap-3 overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-sidebar-ring transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-primary data-[active=true]:font-medium data-[active=true]:text-sidebar-primary-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
...
>(
  (
    ...
    // Wrap children in a span to control visibility
    ...
    const buttonContent = (
        <>
            {icon}
            <span className={cn("whitespace-nowrap overflow-hidden", state === 'collapsed' ? 'hidden' : 'inline')}>
                {text}
            </span>
        </>
    );

    // The button component that will be rendered
    const button = (
      <Comp
        ref={ref}
    ...
    // On mobile, or if no tooltip is provided, just return the button
    if (isMobile || !tooltip) {
      return button
    }
    
    // On desktop, when the sidebar is collapsed, wrap the button in a tooltip
    if (state === "collapsed") {
    ...
    // On desktop, when the sidebar is expanded, just return the button
    return button
  }
)
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  React.useEffect(() => {
    // This now only runs on the client, after initial mount.
...
// Remove SidebarRail export as it's not used in this corrected version
