...
const formSchema = z.object({
    ...
    'address-pj': z.object({
        ...
        zipCode: z.string().min(9, "CEP é obrigatório e deve ter 8 dígitos.").max(9, "CEP inválido"),
    ...
    adminAddress: z.object({
        ...
        zipCode: z.string().min(9, "CEP do sócio é obrigatório e deve ter 8 dígitos.").max(9, "CEP inválido"),
...
export default function PessoaJuridicaPage() {
    ...
    async function onSubmit(values: FormValues) {
        startTransition(async () => {
            ...
            if (result.success) {
                toast({
                    title: "Cadastro Salvo (Simulação)",
                    description: "Redirecionando para a coleta de informações do caso.",
                });
                router.push('/boss/funil-cedrp/coleta-informacoes');
            } else {
                toast({
                    variant: "destructive",
                    title: "Erro na Simulação",
                    description: "A ação de backend simulada falhou.",
                });
            }
        });
    }
