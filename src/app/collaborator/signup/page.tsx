...
export default function SignupCollaboratorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { email, password, name, collaboratorType } = formData;

    if (!email || !password || !name || !collaboratorType) {
      toast({
        variant: 'destructive',
        title: 'Erro de Validação',
        description: 'Nome, e-mail, senha e tipo de colaborador são obrigatórios.',
      });
      setIsLoading(false);
      return;
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/40 py-12">
      <div className="mx-auto w-full max-w-3xl space-y-6 px-4">
            ...
            <CardContent className="pt-6 space-y-8">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-y-4 gap-x-4 md:grid-cols-6">
                        <div className="md:col-span-4"><Label htmlFor="address.street">Rua</Label><Input id="address.street" required onChange={handleInputChange} disabled={isLoading} /></div>
                        <div className="md:col-span-2"><Label htmlFor="address.number">Número</Label><Input id="address.number" required onChange={handleInputChange} disabled={isLoading} /></div>
                        <div className="md:col-span-3"><Label htmlFor="address.complement">Complemento</Label><Input id="address.complement" placeholder="Apto, Bloco, etc." onChange={handleInputChange} disabled={isLoading} /></div>
                        <div className="md:col-span-3"><Label htmlFor="address.neighborhood">Bairro</Label><Input id="address.neighborhood" required onChange={handleInputChange} disabled={isLoading} /></div>
                        <div className="md:col-span-3"><Label htmlFor="address.city">Cidade</Label><Input id="address.city" required onChange={handleInputChange} disabled={isLoading} /></div>
                        <div className="md:col-span-1"><Label htmlFor="address.state">Estado</Label>
                            <Select onValueChange={(value) => handleSelectChange('address.state', value)} disabled={isLoading}><SelectTrigger id="address.state"><SelectValue placeholder="UF" /></SelectTrigger>
                        ...
                        <div className="md:col-span-2"><Label htmlFor="address.zipCode">CEP</Label><Input id="address.zipCode" required onChange={handleInputChange} disabled={isLoading} /></div>
                    </div>
                </div>
                    ...
                    {collaboratorType === 'Estagiário' && (
                         <div className="space-y-4 rounded-md border p-4">
                            <Label className="font-medium">Dados da Faculdade</Label>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="md:col-span-2 space-y-2">
                                    <Label htmlFor="universityDetails.universityName">Nome da Faculdade</Label>
                                    <Input id="universityDetails.universityName" onChange={handleInputChange} disabled={isLoading} />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="universityDetails.universityPhone">Telefone da Faculdade</Label>
                                    <Input id="universityDetails.universityPhone" type="tel" onChange={handleInputChange} disabled={isLoading} />
                                </div>
                                 <div className="space-y-2">
                                    <Label htmlFor="universityDetails.universityEmail">E-mail da Faculdade</Label>
                                    <Input id="universityDetails.universityEmail" type="email" onChange={handleInputChange} disabled={isLoading} />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="universityDetails.currentSemester">Período</Label>
                                    <Input id="universityDetails.currentSemester" type="number" onChange={handleInputChange} disabled={isLoading} />
                                </div>
                                <div className="space-y-2">
                                    <Label>Tipo de Estágio</Label>
                                    <RadioGroup onValueChange={(value) => handleRadioChange('universityDetails.internshipType', value)} className="flex gap-4" disabled={isLoading}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2"><Label htmlFor="bankDetails.accountHolderName">Titular da Conta</Label><Input id="bankDetails.accountHolderName" placeholder="Nome completo do titular" onChange={handleInputChange} disabled={isLoading} /></div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2"><Label htmlFor="bankDetails.pixKeyType">Tipo da Chave Pix</Label>
                            <Select onValueChange={(value) => handleSelectChange('bankDetails.pixKeyType', value)} disabled={isLoading}><SelectTrigger id="bankDetails.pixKeyType"><SelectValue placeholder="Selecione o tipo..." /></SelectTrigger>
                                <SelectContent>
                        ...
                        <div className="space-y-2"><Label htmlFor="bankDetails.pixKey">Chave Pix</Label><Input id="bankDetails.pixKey" placeholder="Sua chave pix" onChange={handleInputChange} disabled={isLoading} /></div>
                    </div>
                    <p className="text-center text-sm text-muted-foreground">Ou informe os dados tradicionais:</p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="space-y-2"><Label htmlFor="bankDetails.bankName">Banco</Label><Input id="bankDetails.bankName" placeholder="Nome do Banco" onChange={handleInputChange} disabled={isLoading} /></div>
                        <div className="space-y-2"><Label htmlFor="bankDetails.agency">Agência</Label><Input id="bankDetails.agency" placeholder="0000" onChange={handleInputChange} disabled={isLoading} /></div>
                        <div className="space-y-2"><Label htmlFor="bankDetails.accountNumber">Conta (com dígito)</Label><Input id="bankDetails.accountNumber" placeholder="00000-0" onChange={handleInputChange} disabled={isLoading} /></div>
                    </div>
                    <div className="space-y-2"><Label>Tipo de Conta</Label>
                        <RadioGroup onValueChange={(value) => handleSelectChange('bankDetails.accountType', value as 'Corrente' | 'Poupança')} className="flex gap-4" disabled={isLoading}>
        ...
        <div className="mt-4 text-center text-sm">
          Já cadastrado?{' '}
          <Link href="/collaborator/login" className="underline" prefetch={false}>
            Voltar para o Login
          </Link>
        </div>
      </div>
